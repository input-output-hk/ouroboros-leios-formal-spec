...
module MAlonzo.Code.Leios.Foreign.HsTypes where
...
instance Monad (ComputationResult e) where
  ...
  (Success a) >>= m = m a
  (Failure e) >>= _ = Failure e
-- Leios.Foreign.HsTypes.Empty
d_Empty_6 = ()
type T_Empty_6 = Void
cover_Empty_6 :: Void -> ()
cover_Empty_6 x = case x of
-- Leios.Foreign.HsTypes.Rational
d_Rational_8 = ()
...
cover_HSMap_16 x
  = case x of
      MkHSMap _ -> ()
-- Leios.Foreign.HsTypes.HSMap.assocList
d_assocList_24 ::
...
cover_HSSet_30 x
  = case x of
      MkHSSet _ -> ()
-- Leios.Foreign.HsTypes.HSSet.elems
d_elems_36 :: T_HSSet_30 AgdaAny -> [AgdaAny]
d_elems_36 v0
  = case coe v0 of
      C_MkHSSet_38 v1 -> coe v1
      _ -> MAlonzo.RTE.mazUnreachableError
-- Leios.Foreign.HsTypes.ComputationResult
