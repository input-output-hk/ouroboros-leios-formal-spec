...
data _-⟦_/_⟧ˢ⇀_ : MachineType Network FFD ShimState where
  ...
  Progress : let open ShimState s in
    ...
    ∙ iteration ≤ 3
    ─────────────────────────────────────────
    s -⟦ L⊗ ϵ ↑ᵢ m / just (L⊗ ϵ ↑ₒ SLOT) ⟧ˢ⇀ record { iteration = suc iteration ; buffer = buffer ++ messages m }

  Finish : let open ShimState s in
    ∙ iteration ≡ 4
    ─────────────────────────────────────────
    s -⟦ L⊗ ϵ ↑ᵢ m / just (ϵ ⊗R ↑ₒ Done (buffer ++ messages m)) ⟧ˢ⇀ record { iteration = 0 ; buffer = [] }
