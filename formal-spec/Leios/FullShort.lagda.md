...
module Leios.FullShort (⋯ : SpecStructure 1)
...
<!--
```agda
{- Step relation for block production.
-}
```
-->
```agda
...
```agda
  EB-Role : let open LeiosState s renaming (FFDState to ffds)
                ...
                LE = map getEBRef $ filter (λ eb → isVoteCertified s eb × eb ∈ᴮ slices L slot (3 * η / L) 2) EBs
