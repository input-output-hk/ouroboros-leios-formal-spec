...
SFunType : (∀ {a} → Type a → Type a) → Type → Type → Type → Type
SFunType M A B S = S × A → M (S × B)

-- explicit state
record SFunᵉ (M : ∀ {a} → Type a → Type a) (A B : Type) : Type₁ where
  field
    State : Type
    init  : State
    fun   : SFunType M A B State

private variable
  A B C D State : Type

trace : ∀ {n} {M : ∀ {a} → Type a → Type a} ⦃ mon : Monad M ⦄ → SFunType M A B State → State → Vec A n → M (Vec B n)
trace _ _ [] = return []
trace f s (a ∷ as) = do
  (s , b) ← f (s , a)
  bs ← trace f s as
...
-- let (s , b) = f (s , a) in b ∷ trace f s as
