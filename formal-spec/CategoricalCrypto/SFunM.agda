{-# OPTIONS --safe #-}
module CategoricalCrypto.SFunM where
...
module _ {M : Type↑} ⦃ Monad-M : Monad M ⦄ ⦃ M-Laws : MonadLaws M ⦄ where
  ...
  -- trace with state
  strace : ∀ {n} → SFunType A B State → (State × Vec A n) → M (State × Vec B n)
  strace f (s , []) = return (s , [])
  strace f (s , (a ∷ as)) =
    f (s , a) >>= λ (s , b) → strace f (s , as) >>= λ (s , bs) → return (s , b ∷ bs)

  strace-app : ∀ {n m} → SFunType A B State → State → Vec A n → Vec A m → M (State × Vec B (n +ℕ m))
  strace-app f s as as' =
    strace f (s , as) >>= λ (s , bs) → strace f (s , as') >>= λ (s , bs') → return (s , bs ++ bs')

  open ≡-Reasoning

  infixr 4 _⟩>>=⟨_ refl⟩>>=⟨_
  infixl 5 _⟩>>=⟨refl
  _⟩>>=⟨_ : ∀ {x y : M A} {f g : A → M B} → x ≡ y → f ≗ g → (x >>= f) ≡ (y >>= g)
  _⟩>>=⟨_ = {!!} -- this may require funext

  refl⟩>>=⟨_ : ∀ {x : M A} {f g : A → M B} → f ≗ g → (x >>= f) ≡ (x >>= g)
  refl⟩>>=⟨_ = {!!}

  _⟩>>=⟨refl : ∀ {x y : M A} {f : A → M B} → x ≡ y → (x >>= f) ≡ (y >>= f)
  _⟩>>=⟨refl = {!!}

  strace-cont : ∀ {m n} {f : SFunType A B State} {s} {as : Vec A n} {as' : Vec A m}
             → strace f (s , as ++ as') ≡ strace-app f s as as'
  strace-cont {f = f} {s} {[]} {as'} = begin
    strace f (s , as')
      ≡⟨ sym (>>=-identityʳ (strace f (s , as'))) ⟩
    (strace f (s , as') >>= return)
      ≡⟨⟩
    (strace f (s , as') >>= λ (s , bs') → return (s , [] ++ bs'))
      ≡⟨ sym >>=-identityˡ ⟩
    (return (s , []) >>= λ (s , bs) → strace f (s , as') >>= λ (s , bs') → return (s , bs ++ bs'))
      ≡⟨⟩
    strace-app f s [] as' ∎

  strace-cont {f = f} {s} {a ∷ as} {as'} = begin
    strace f (s , (a ∷ as) ++ as')
      ≡⟨⟩
    (f (s , a) >>= λ (s , b) → strace f (s , as ++ as') >>= λ (s , bs) → return (s , b ∷ bs))
      ≡⟨ refl⟩>>=⟨ (λ (s , b) → strace-cont {as = as} {as' = as'} ⟩>>=⟨refl) ⟩
    (f (s , a) >>= λ (s , b) → strace-app f s as as' >>= λ (s , bs) → return (s , b ∷ bs))
      ≡⟨⟩
    (f (s , a) >>= λ (s , b) → (strace f (s , as) >>= λ (s , bs) → strace f (s , as') >>= λ (s , bs') → return (s , bs ++ bs')) >>= λ (s , bs) → return (s , b ∷ bs))
      ≡⟨ refl⟩>>=⟨ (λ (s , b) → {!!}) ⟩
    (f (s , a) >>= λ (s , b) → strace f (s , as) >>= λ (s , bs) → return (s , b ∷ bs) >>=
        (λ (s , bs) → strace f (s , as') >>= λ (s , bs') → return (s , bs ++ bs')))
      ≡⟨ sym {!>>=-assoc (f (s , a))!} ⟩
    ((f (s , a) >>= (λ (s , b) → strace f (s , as) >>= λ (s , bs) → return (s , b ∷ bs))) >>=
        (λ (s , bs) → strace f (s , as') >>= λ (s , bs') → return (s , bs ++ bs')))
      ≡⟨⟩
    (strace f (s , a ∷ as) >>= λ (s , bs) → strace f (s , as') >>= λ (s , bs') → return (s , bs ++ bs'))
      ≡⟨⟩
    strace-app f s (a ∷ as) as' ∎

  -- trace : ∀ {n} → SFunType A B State → State → Vec A n → M (Vec B n)
  -- trace f s [] = return []
  -- trace f s (a ∷ as) = do
  --   (s , b) ← f (s , a)
  --   b ∷_ <$> trace f s as

  -- trace-cont : ∀ {m n} {f : SFunType A B State} {s} {as : Vec A n} {as' : Vec A m}
  --            → Σ[ bs' ∈ M (Vec B m) ] trace f s (as ++ as') ≡ _++_ <$₂> trace f s as ⨾ bs'
  -- trace-cont {f = f} {s} {[]} = {!!}
  -- trace-cont {f = f} {s} {a ∷ as} = {!!}
