...
-----------------------------------------------------------------------------------------------------
-- Concrete machines with partial function and initial states

record SRel' (A B : Type) : Type₁ where
  field
    S : Type           -- type of states
    P : (S × A) → Type -- states from which we can transition
    i : S              -- current State
    f : ∃ P → (B × S)  -- transition function

  data Transition (a : A) : SRel' A B → B → Type where
    transition : (p : P (i , a)) → let b , s = f (-, p) in Transition a record { S = S ; P = P ; i = s ; f = f } b

open SRel'

data Trace' {A B} (rel : SRel' A B) : List A → List B → Type₁ where
  trace[] : Trace' rel [] []
  trace∷ : ∀ {a b as bs rel'} → Transition rel a rel' b → Trace' rel' as bs → Trace' rel (a ∷ as) (b ∷ bs)

--   trace∷ : ∀ {s s' s'' a b as bs} → rel s a b s' → Trace rel s' as bs s'' → Trace rel s (a ∷ as) (b ∷ bs) s''
...
module _ (A B : Type) where
  ...
  ------------------------------------------------------------------------
  -- Machines seen from their traces. A machine here is only represented by the
  -- set of admissible traces, i.e. the traces that are permitted by the
  -- execution on the machines.

  -- A trace is simple or list of tuples. The first element of the tuple (of
  -- type A) is the received signal, and the seconde, of type B, is the sent
  -- signal in response.
  trace : Type
  trace = List A × List B
  
  Machine-Trace : Type₁
  Machine-Trace = Pred trace _

  _≤ᵗ_ : Rel Machine-Trace _
  _≤ᵗ_ = _⊆_

  -- Comparing abstract machines means comparing the subsets of admissible traces
  _≡ᵗ_ : Rel Machine-Trace _
  _≡ᵗ_ = _≐_

  ------------------------------------------------------------------------
  -- Machines seen as relational objects. Machines maintain an internal state,
  -- which, in turn, is used to dictate which output signals are permitted when
  -- a given input signal is received.

  SRelType : Type → Type₁
  SRelType S = S → A → B → S → Type

  SRel : Type₁
  SRel = ∃ SRelType


data Trace {A B S : Type} (rel : SRelType A B S) : SRelType (List A) (List B) S where
  trace[] : ∀ {s} → Trace rel s [] [] s
  trace∷ : ∀ {s s' s'' a b as bs} → rel s a b s' → Trace rel s' as bs s'' → Trace rel s (a ∷ as) (b ∷ bs) s''

admissible : ∀ {A B : Type} → SRel A B → trace A B → Type
admissible (S , sRel) (as , bs) = ∃ λ s → ∃ λ s' → Trace sRel s as bs s'

is-admissible : ∀ {A B : Type} → SRel A B → Machine-Trace A B
...
-- bisimilarFromOn : (as : List A) (bs : List B) (R₁ R₂ : SRel A B) (s₁ : proj₁ R₁) (s₂ : proj₁ R₂) → Type
-- bisimilarFromOn as bs R₁ R₂ s₁ s₂ = Trace R₁ s₁ as bs ⇔ Trace R₂ s₂ as bs

-- bisimilarFrom : (R₁ R₂ : SRel A B) (s₁ : proj₁ R₁) (s₂ : proj₁ R₂) → Type
-- bisimilarFrom R₁ R₂ s₁ s₂ = ∀ as bs → bisimilarFromOn as bs R₁ R₂ s₁ s₂

-- bisimilar∷ : {R₁ R₂ : SRel A B} {s₁ s₁' : proj₁ R₁} {s₂ s₂' : proj₁ R₂} {a : A} {b : B} →
--   proj₂ R₁ s₁ a b s₁' → proj₂ R₂ s₂ a b s₂' → bisimilarFrom R₁ R₂ s₁ s₂ → bisimilarFrom R₁ R₂ s₁' s₂'
-- bisimilar∷ {a = a} {b} p₁ p₂ bisim as bs = mk⇔
--   (λ x → let y =  bisim (a ∷ as) (b ∷ bs) .Equivalence.to (trace∷ p₁ x) in case y of λ where
--     (trace∷ x x₁) → {!x₁!})
--   {!!}

-- _≤ᵗ_ : SRel A B → SRel A B → Type
-- R₁ ≤ᵗ R₂ = ∀ s₁ → ∃[ s₂ ] bisimilarFrom R₁ R₂ s₁ s₂
...
--     ≡-≡-trace⇔ : ∀ {s s' as bs} → s' ≡ to s → bisimilarFromOn as bs R₁ R₂ s s'
...
-- ∘-respects-≤ᵗ : ∀ {A B C} {R₁ R₂ : SRel B C} {R₃ R₄ : SRel A B} → R₁ ≤ᵗ R₂ → R₃ ≤ᵗ R₄ → (R₁ ∘ R₃) ≤ᵗ (R₂ ∘ R₄)
-- ∘-respects-≤ᵗ {A} {B} {C} {R₁} {R₂} {R₃} {R₄} R₁≤ᵗR₂ R₃≤ᵗR₄ (s₁ , s₃) with R₁≤ᵗR₂ s₁ | R₃≤ᵗR₄ s₃
-- ... | s₂ , trace₁≡trace₂ | s₄ , trace₃≡trace₄ = (s₂ , s₄) , aux s₁ s₂ s₃ s₄ trace₁≡trace₂ trace₃≡trace₄
--   where
--     aux : ∀ s₁ s₂ s₃ s₄ → bisimilarFrom R₁ R₂ s₁ s₂ → bisimilarFrom R₃ R₄ s₃ s₄ → bisimilarFrom (R₁ ∘ R₃) (R₂ ∘ R₄) (s₁ , s₃) (s₂ , s₄)
--     aux _  _  _  _  _             _             []       []       = mk⇔ (const trace[]) (const trace[])
--     aux _  _  _  _  _             _             []       (_ ∷ _ ) = mk⇔ (λ ()) (λ ())
--     aux _  _  _  _  _             _             (a ∷ as) []       = mk⇔ (λ ()) (λ ())
--     aux s₁ s₂ s₃ s₄ trace₁≡trace₂ trace₃≡trace₄ (a ∷ as) (c ∷ cs) = mk⇔
--         (λ where
--           (trace∷ {next = s₁' , s₃'} record { b = b ; ∘-rel₁ = ∘-rel₁ ; ∘-rel₂ = ∘-rel₂ } t) →
--             let t₁→t₂[b,c] = trace₁≡trace₂ (b ∷ []) (c ∷ []) .Equivalence.to
--                 t₃→t₄[a,b] = trace₃≡trace₄ (a ∷ []) (b ∷ []) .Equivalence.to
--              in case (t₁→t₂[b,c] (trace∷ ∘-rel₁ trace[]),
--                       t₃→t₄[a,b] (trace∷ ∘-rel₂ trace[])) of
--                λ where
--                  (trace∷ {next = s₂'} r₁ trace[] , trace∷ {next = s₄'} r₂ trace[]) →
--                    trace∷ {next = s₂' , s₄'} (record { b = b ; ∘-rel₁ = r₁ ; ∘-rel₂ = r₂ })
--                      (aux s₁' s₂' s₃' s₄' (bisimilar∷ ∘-rel₁ r₁ trace₁≡trace₂) (bisimilar∷ ∘-rel₂ r₂ trace₃≡trace₄) as cs .Equivalence.to t) )
--         λ where
--           (trace∷ x x₁) → {!!}

-- ∘-respects-≡ᵗ : R₁ ≡ᵗ R₂ → R₃ ≡ᵗ R₄ → (R₁ ∘ R₃) ≡ᵗ (R₂ ∘ R₄)
-- ∘-respects-≡ᵗ (R₁≤ᵗR₂ , R₂≤ᵗR₁) (R₃≤ᵗR₄ , R₄≤ᵗR₃) =
--   (λ where
--      (s₁ , s₃) → {!!}) ,
--   {!!}
-- --  (λ {(s₁ , s₃) → {!!} })
...
--     --    (λ where
--     --       trace[] → trace[]
--     --       (trace∷ {next = (o , p)} record { b = b ; ∘-rel₁ = ∘-rel₁ ; ∘-rel₂ = ∘-rel₂ } x₁) → trace∷ {next = {!snd!} , {!!}} {!!} {!!})
--     --    {!!}}) ,
-- --  (λ {s₁ → {!!}})
...
-- ≡ⁱ⇒≡ᵗ (≡ᵉ⇒≡ⁱ x) = ≡ᵉ⇒≡ᵗ x
-- ≡ⁱ⇒≡ᵗ weaken = {!!}
-- ≡ⁱ⇒≡ᵗ (≡ⁱ-∘ x x₁) = ∘-respects-≡ᵗ (≡ⁱ⇒≡ᵗ x) (≡ⁱ⇒≡ᵗ x₁)
-- ≡ⁱ⇒≡ᵗ (≡ⁱ-⊗ x x₁) = ⊗₁-respects-≡ᵗ (≡ⁱ⇒≡ᵗ x) (≡ⁱ⇒≡ᵗ x₁)
-- ≡ⁱ⇒≡ᵗ ≡ⁱ-refl = ≡ᵗ-reflexive
-- ≡ⁱ⇒≡ᵗ (≡ⁱ-sym x) = let (u , v) = ≡ⁱ⇒≡ᵗ x in (v , u)
-- ≡ⁱ⇒≡ᵗ (≡ⁱ-trans x x₁) = ≡ᵗ-transitive (≡ⁱ⇒≡ᵗ x) (≡ⁱ⇒≡ᵗ x₁)
